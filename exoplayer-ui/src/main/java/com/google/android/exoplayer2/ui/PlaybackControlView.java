...
import me.happycao.exoplayer.ui.R;
...
public class PlaybackControlView extends FrameLayout {
  ...
  // 自定义
  private final LinearLayout exoTitleBar;
  private final View backButton;
  private final TextView titleView;
  private final View playPreviousButton;
  private final View playNextButton;
  private @DrawableRes int exoFullscreenSelector = R.drawable.exo_fullscreen_selector;
  ...
  public PlaybackControlView(Context context, AttributeSet attrs, int defStyleAttr,
    ...
    if (playbackAttrs != null) {
      ...
      try {
        ...
        exoFullscreenSelector = a.getResourceId(R.styleable.PlaybackControlView_player_fullscreen_selector, exoFullscreenSelector);
      } finally {
        a.recycle();
      }
    }
    ...
    // 自定义
    exoTitleBar = findViewById(R.id.exo_title_bar);
    ...
    playPreviousButton = findViewById(R.id.exo_play_prev);
    if (playPreviousButton != null) {
      playPreviousButton.setVisibility(View.GONE);
      playPreviousButton.setOnClickListener(componentListener);
    }
    playNextButton = findViewById(R.id.exo_play_next);
    if (playNextButton != null) {
      playNextButton.setVisibility(View.GONE);
      playNextButton.setOnClickListener(componentListener);
    }
    exoFullscreen = findViewById(R.id.exo_fullscreen);
    if (exoFullscreen != null) {
      exoFullscreen.setButtonDrawable(exoFullscreenSelector);
      exoFullscreen.setOnCheckedChangeListener(componentListener);
    }
  ...
  public View getPlayPreviousButton() {
    return playPreviousButton;
  }

  public View getPlayNextButton() {
   ...
   * 设置全屏按钮样式
   *
   * @param exoFullscreenStyle 全屏按钮样式
   */
  public void setExoFullscreenStyle(@DrawableRes int exoFullscreenStyle) {
    this.exoFullscreenSelector = exoFullscreenStyle;
    if (getExoFullscreen() != null) {
      getExoFullscreen().setButtonDrawable(exoFullscreenStyle);
    }
  }
   ...
   * 获取全屏按钮样式
   */
  public int getExoFullscreenSelector() {
    return this.exoFullscreenSelector;
  }

  public LinearLayout getExoTitleBar() {
    return this.exoTitleBar;
  }
  ...
  private void updateProgress() {
    ...
    if (durationView != null) {
      durationView.setText(getStringForTime(formatBuilder, formatter, duration));
    }
    if (positionView != null && !scrubbing) {
      positionView.setText(getStringForTime(formatBuilder, formatter, position));
    }
    if (timeBar != null) {
  ...
  private final class ComponentListener implements Player.EventListener, TimeBar.OnScrubListener,
      ...
      if (player != null) {
        if (nextButton == view) {
          ...
          if (backListener != null) {
            if (exoFullscreen.isChecked()) {
              exoFullscreen.setChecked(false);
            }
            backListener.OnBack();
          }
        } else if (playPreviousButton == view) {
          if (controlListener != null) {
            controlListener.onPrevious();
          }
        } else if (playNextButton == view) {
          if (controlListener != null) {
    ...
    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
      if (fullScreenChangeListener != null) {
        fullScreenChangeListener.OnFullScreenChange(isChecked);
      }
    }
  }
}
